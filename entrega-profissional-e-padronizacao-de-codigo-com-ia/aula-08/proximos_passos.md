...
- [x] `ruff check` e `ruff format --check` limpos em `server/` e `tests/`
- [x] `pytest` verde — cobre tools/helpers, **não** texto do LLM
- [x] `python verificar_pipeline.py` retorna **0**
- [x] `docker build` + container sobe (sandbox `:8800`)
- [x] Workflow em `ci/` espelha o portão local
- [ ] PR com CI verde (Opcional para os Alunos)
