"""Gera relatório comparando snapshots 'antes' com o estado atual."""
...
MODULO = BASE / "cobranca_service.py"
...
def main() -> int:
    if not METRICAS_ANTES.exists() or not COMPORTAMENTO_ANTES.exists():
        print("Snapshots 'antes' ausentes em snapshots/. Rode: python gerar_snapshots_antes.py")
        return 1
    ...
    comportamento_depois = capturar(cobranca.calcular_cobranca)

    print("=== Avaliacao antes vs depois ===\n")
    print("Metricas (cobranca_service.py):")
    ...
    if comportamento_depois == comportamento_antes:
        ...
        for registro in comportamento_depois:
            ...
            if registro["saida"] != antes_por_id.get(cid):
                print(f"  - {cid}")
                print(f"      antes:  {antes_por_id.get(cid)}")
                print(f"      depois: {registro['saida']}")
