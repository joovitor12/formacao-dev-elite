...
### 1. No exercício de comentários automáticos, qual é o objetivo principal da triagem no inventário?
   - A) Aplicar no merge todas as sugestões de bots para manter o pipeline sempre verde.
   - B) Catalogar fonte e tipo de cada comentário e decidir o que vira ação, dismiss ou ruído.
   - C) Desativar linters no repositório para reduzir fadiga do autor do PR.
   - D) Converter comentários humanos em comentários automáticos para padronizar o tom.

### 2. Dois bots apontam o mesmo smell (ex.: segredo em log) com redações diferentes. A postura mais alinhada ao material é:
   - A) Ignorar os dois porque duplicação prova que é falso positivo.
   - B) Tratar como um único achado na triagem e evitar que o autor corrija duas vezes por fadiga.
   - C) Exigir que cada bot remova o comentário antes de qualquer análise humana.
   - D) Bloquear merge só quando três ou mais ferramentas repetirem o mesmo texto.
...
### 3. Um comentário automático de linter sobre variável não usada, sem impacto em segurança ou operação em produção, costuma ser classificado como:
   ...
   - B) Ruído ou parcial — candidato a dismiss se não houver risco real no contexto do PR.
   - C) Crítico de compliance — linters sempre detectam exposição de dados pessoais.
   - D) Inexistente — linters nunca postam comentário inline no diff do GitHub.
...
### 5. Na detecção de riscos, a diferença entre **code smell** e **risco** no material é melhor descrita como:
   - A) Smell e risco são sinônimos — qualquer nit de estilo bloqueia deploy em produção.
   - B) Smell é indício de desenho frágil; risco é impacto potencial em produção se o merge ocorrer.
...
### 6. PII (ex.: e-mail de cliente) incluído em log de deploy canário deve ser mapeado principalmente como risco de:
   - A) Manutenção — renomear variáveis resolve exposição em observabilidade.
   - B) Compliance — dado sensível em trilha de log pode violar política e gerar incidente.
   - C) Escopo — logs não fazem parte do diff e não entram no mapa de riscos.
   - D) Performance — e-mail em log só afeta latência, não governança de dados.

### 7. “Lacuna de detecção” no mapa de riscos refere-se a:
   - A) Comentário duplicado entre Copilot e linter no mesmo trecho do diff.
   - B) Risco relevante que ferramentas não sinalizaram (ou sinalizaram mal) e o humano precisa registrar.
   - C) Falha de rede ao carregar a página do PR no navegador do revisor.
...
### 8. Um PR altera abort de canário de `taxa_erro >= 0,05` para `taxa_erro > 0,05` em produção. No mapa de riscos, isso tende a ser:
   - A) Nit de estilo — comparações numéricas não afetam comportamento operacional.
   - B) Risco de operação/correção — na fronteira exata o abort pode deixar de disparar.
...
### 6. Resposta: B
**Justificativa:** Exposição de e-mail em log é exemplo de risco de compliance/PII, não mera questão de nomenclatura.
